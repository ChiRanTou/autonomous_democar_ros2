...
MpcController::MpcController(string name) : Node(name)
{
    ...
    loadPathInfo();
    //设置订阅者和发布者
    pub_path = this->create_publisher<nav_msgs::msg::Path>("base_path",1);      //路径发布者
    pub_cmd = this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel",1);   //指令发布者
    sub_odom = this->create_subscription<nav_msgs::msg::Odometry>("odom",1,bind(&MpcController::odom_callback,this,_1));        //里程计订阅者
}
...
int main(int argc,char **argv)
{
    ...
    //创建节点
    auto node = std::make_shared<MpcController>("mpc_controller");
