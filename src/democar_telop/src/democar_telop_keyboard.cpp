...
class DemocarTelopKeyBoard : public rclcpp::Node
{
    public:
        //构造函数
        DemocarTelopKeyBoard(string name);
...
int main(int argc,char **argv)
{
    ...
    //创建主程序节点
    auto node = std::make_shared<DemocarTelopKeyBoard>("democar_telop_keyboard");
...
/*  对对象中的函数进行定义  */
//构造函数定义
DemocarTelopKeyBoard::DemocarTelopKeyBoard(string name):Node(name)
{
    RCLCPP_INFO(this->get_logger(),"Starting up democar telop keyboard controller");
...
void DemocarTelopKeyBoard::get_key()
{
    auto KBC = KeyboardCtrl();
    auto key = KBC.get_press_key();
    RCLCPP_INFO(this->get_logger(),"get keyboard press 0x%02X \n",key);
}
